...
export async function GET(req: NextRequest) {
  ...
  if (requestedCompanyId && requestedCompanyId !== authUser.companyId && authUser.role !== "PLATFORM_ADMIN") {
    return NextResponse.json({ success: false, error: "Forbidden: Cross-tenant executive data access blocked" }, { status: 403 });
  }
  ...
  try {
    ...
    return NextResponse.json({ success: false, error: error.message || "Failed to fetch executive overview" }, { status: 500 });
  }
}
