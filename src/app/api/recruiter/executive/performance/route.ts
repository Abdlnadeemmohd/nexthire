...
import { analyzeTimeToHire, forecastRecruiterCapacity, getCostAndRoiIntelligence } from "@/lib/executive";
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    return NextResponse.json({ success: false, error: error.message || "Failed to fetch performance metrics" }, { status: 500 });
  }
}
