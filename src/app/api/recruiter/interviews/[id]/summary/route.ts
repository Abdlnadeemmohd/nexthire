...
import { generateInterviewSummary } from "@/lib/interview/decisionEngine";
...
export async function GET(
  ...
  try {
    ...
    return NextResponse.json({ success: false, error: error.message || "Failed to fetch summary" }, { status: 500 });
  }
}
...
export async function POST(
  ...
  try {
    ...
    const summary = await generateInterviewSummary(id, companyId);
    return NextResponse.json({ success: true, data: summary });
  } catch (error: any) {
    return NextResponse.json({ success: false, error: error.message || "Failed to generate interview summary" }, { status: 500 });
  }
}
